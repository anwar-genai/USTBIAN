...
export default function FollowersPage() {
  ...
  const loadFollowers = async () => {
    try {
      ...
      // Load followers
      const followersData = await api.getFollowers(token, foundUser.id);
        ...
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-600 mt-1">
              People following <span className="font-semibold">{user.displayName}</span>
            </p>
          </div>
          ...
          <div className="divide-y divide-gray-200">
            {followers.length === 0 ? (
              <div className="p-8 text-center text-gray-500">
                ...
                <p className="text-sm">When people follow {user.displayName}, they'll appear here.</p>
              </div>
            ) : (
              followers.map((follower) => (
                <div key={follower.id} className="p-6 hover:bg-gray-50 transition">
                  <div className="flex items-center gap-4">
                    <Link href={`/user/${follower.username}`} className="flex-shrink-0">
                      {follower.avatarUrl ? (
                        <img
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        <p className="font-semibold text-gray-900 truncate">{follower.displayName}</p>
                        <p className="text-sm text-gray-600 truncate">@{follower.username}</p>
                        {follower.bio && (
                          <p className="text-sm text-gray-500 mt-1 line-clamp-2">{follower.bio}</p>
                        )}
                      </Link>
                    </div>
